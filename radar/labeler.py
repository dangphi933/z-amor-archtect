        ...
        for trade in trades:
            try:
                ...
                scan = db.execute(text("""
                    SELECT
                        scan_id, score, regime,
                        CAST(breakdown->>'trend_strength' AS numeric)     AS trend_strength,
                        CAST(breakdown->>'volatility_quality' AS numeric) AS vol_quality,
                        CAST(breakdown->>'session_bias' AS numeric)       AS session_bias,
                        CAST(breakdown->>'market_structure' AS numeric)   AS mkt_structure,
                        CAST(breakdown->>'adx_live' AS numeric)           AS adx_live,
                        CAST(breakdown->>'rsi_live' AS numeric)           AS rsi_live,
                        CAST(breakdown->>'atr_pct_live' AS numeric)       AS atr_pct_live,
                        utc_hour, utc_dow
                    FROM radar_scans
                    WHERE asset = :asset
