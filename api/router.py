...
# ── Symbol → Asset mapping (dùng cho EA heartbeat lookup) ─────────────────────
# Key: MT5 symbol name (có thể có suffix .m, .raw, etc.)
# Val: Asset name theo engine.py ASSET_PROFILES
SYMBOL_TO_ASSET: dict = {
    # GOLD
    "XAUUSD":  "GOLD", "XAUUSDm": "GOLD", "XAUUSD.": "GOLD",
    "GOLD":    "GOLD", "GOLDm":   "GOLD",
    # EURUSD
    "EURUSD":  "EURUSD", "EURUSDm": "EURUSD", "EURUSD.": "EURUSD",
    # BTC
...
async def radar_scan(req: RadarScanRequest, db: Session = Depends(get_db)):
    ...
    try:
        db.execute(text("""
            INSERT INTO radar_scans
              (scan_id, asset, timeframe, email, session, utc_hour, utc_dow,
               ...
               allow_trade, position_pct, sl_multiplier, state_cap)
            VALUES
        ...
        """), {
            ...
            "data_source":  result.breakdown.get("data_source", "fallback"),
            # Phase 4B: EA integration fields
            "allow_trade":  result.ea_allow_trade,
            "position_pct": result.ea_position_pct,
            "sl_multiplier":result.ea_sl_multiplier,
            "state_cap":    result.ea_state_cap,
        })
        db.commit()
    ...
    return RadarScanResponse(
        ...
        risk_level    = result.risk_level,
        session       = result.session,
        cta_url       = f"{BASE_URL}/scan?asset={result.asset.lower()}&tf={result.timeframe}",
        share_url     = f"{BASE_URL}/scan?asset={result.asset.lower()}&tf={result.timeframe}",
...
async def radar_result_page(scan_id: str, db: Session = Depends(get_db)):
    row = None
    try:
        row = db.execute(text("SELECT * FROM radar_scans WHERE scan_id = :id"), {"id": scan_id}
        ).fetchone()
    except Exception:
        pass
    ...
    try:
        db.execute(text("UPDATE radar_scans SET result_viewed = TRUE WHERE scan_id = :id"),
            {"id": scan_id}
        )
        db.commit()
    except Exception:
...
# ── Utility: get_cached_radar — dùng bởi scheduler.py ────────────────────────
_radar_cache: dict = {}  # {asset/tf: {result_dict, expires_at}}

def get_cached_radar(asset: str, tf: str, force_refresh: bool = False) -> dict:
    """
    Compute radar cho asset/tf, cache in-memory theo TTL.
    Scheduler gọi force_refresh=True mỗi 30 phút.
    Heartbeat đọc từ cache này → không compute lại mỗi 30s.
    """
    from datetime import datetime, timezone
    ...
    if not force_refresh and key in _radar_cache:
        cached = _radar_cache[key]
