...
async def radar_stats(db: Session = Depends(get_db)):
    try:
        ...
        today   = db.execute("SELECT COUNT(*) FROM radar_scans WHERE created_at > NOW() - INTERVAL '24 hours'").scalar()
        by_asset = db.execute("""
            SELECT asset, COUNT(*) as cnt, AVG(score) as avg_score
            FROM radar_scans GROUP BY asset ORDER BY cnt DESC
        """).fetchall()
        convs   = db.execute("SELECT COUNT(*) FROM radar_scans WHERE converted = TRUE").scalar()
    except Exception as e:
        raise HTTPException(500, str(e))
    return {
...
async def track_cta(scan_id: str, db: Session = Depends(get_db)):
    try:
        db.execute("UPDATE radar_scans SET cta_clicked = TRUE WHERE scan_id = :id", {"id": scan_id})
        db.commit()
    except Exception:
        pass
    return {"ok": True}
